...
(*
  Horse + mORMot2 Provider  —  Integration Test Client
  =====================================================
  Destination: horse-provider-mormot/samples/tests/HorseMormotTestClient.dpr

  Requires HorseMormotConsoleTestServer (or any mORMot2-backed Horse server)
  running on 127.0.0.1:9010 before executing.

  Test matrix (1–32 identical to HorseCSTestClient; 33–36 are the streaming
  tests — same structure as the CrossSocket suite, adapted: mORMot2 returns
  501 for all /stream/* routes because it does not implement chunked streaming):
    ...
    33  GET    /stream/pull                  → 501 not-implemented + server still healthy
                                               (mORMot2 does not support SendChunked)
...
var
  ...
  LSeqMarker:     string;
  J:              Integer;
  // Tests 36 — concurrent streaming probe (expects 501 on mORMot2)
  ...
  // On mORMot2 the server returns 501 synchronously; both concurrent probes
...
begin
  ...
  // ── 09 ───────────────────────────────────────────────────────────────────────
  Section('09  GET /params/query?name=Horse&value=mORMot');
  DoSync(AClient, 'GET',
    BASE_URL + '/params/query?name=Horse&value=mORMot', nil, nil, R);
  Check('status 200',            R.StatusCode = 200, IntToStr(R.StatusCode));
  Check('name = "Horse"',        Pos('"Horse"',  R.Body) > 0, R.Body);
  Check('value = "mORMot"',      Pos('"mORMot"', R.Body) > 0, R.Body);

  // ── 10  FIX-HEADER-DUP: two Set-Cookie headers ───────────────────────────────
  // Uses Res.AddHeader('Set-Cookie', ...) twice.  Before FIX-HEADER-DUP, the
  // TDictionary-backed CustomHeaders silently overwrote the first cookie with
  // the second.  After the fix (TStringList), both cookies survive.
  ...
  // ── 15 ───────────────────────────────────────────────────────────────────────
  Section('15  POST /methods/post  (empty body)');
  LHeaders := THttpHeader.Create;
  try
    LHeaders['Content-Type'] := 'application/json; charset=utf-8';
    DoSync(AClient, 'POST', BASE_URL + '/methods/post',
  ...
  Check('step B: body does NOT contain SEQUENTIAL_BODY_ALPHA',
    Pos('SEQUENTIAL_BODY_ALPHA', R.Body) = 0, R.Body);
  ...
  Check('no response contains another request''s marker', LNoCrossLeaks, '');
  ...
  // ── 25 ───────────────────────────────────────────────────────────────────────
  Section('25  GET /raw/webrequest  (RawWebRequest adapter)');
  LHeaders := THttpHeader.Create;
  try
  ...
  // ── 30 ───────────────────────────────────────────────────────────────────────
  Section(Format('30  POST /pool/burst  %d rapid sequential after burst',
    [RAPID_SEQ_COUNT]));
  LSeqAllOk := True;
  ...
  // mORMot2 uses THttpServer's synchronous response model and does not expose
  // its IOCP/epoll pump for application-level chunked streaming.  The server
  // registers a 501 stub so the capability flag can be detected at runtime.
  // Mirrors test 33 in HorseCSTestClient (which expects 200 + chunked body).
  Section('33  GET /stream/pull  (mORMot2: streaming not supported → 501)');
  DoSync(AClient, 'GET', BASE_URL + '/stream/pull', nil, nil, R);
  Check('status 501 (streaming not implemented on mORMot2 transport)',
  ...
  // On mORMot2 the /stream/content-type stub also returns 501.
  Section('34  GET /stream/content-type  (mORMot2: streaming not supported → 501)');
  ...
  Section('35  GET /stream/empty  (mORMot2: streaming not supported → 501)');
  ...
  // On mORMot2 both return 501 synchronously; the test confirms neither hangs,
  // both produce the 501 status code, and the server is still healthy after.
  Section('36  GET /stream/pull  ×2 concurrent  (mORMot2: both → 501, server healthy)');
  for I := 0 to 1 do
  begin
...
begin
  Writeln('[HorseMormotTest] Client — target: ' + BASE_URL);
  Writeln('[HorseMormotTest] Ensure a mORMot2 Horse server is running on port 9010.');
  Writeln('[HorseMormotTest] (e.g. HorseMormotConsoleTestServer.exe)');
  Writeln('');
  ...
  try
    try
      ...
      on E: Exception do
        Writeln('[HorseMormotTest] Unexpected exception: ' + E.ClassName
          + ': ' + E.Message);
    end;
  finally
    Client.Free;
  ...
  Writeln('');
  Writeln(Format('[HorseMormotTest] %d passed, %d failed  (total %d) in %d ms wall clock',
    [GPassCount, GFailCount, GPassCount + GFailCount,
     GGlobalSW.ElapsedMilliseconds]));
  if GFailCount = 0 then
    Writeln('[HorseMormotTest] All tests PASSED.')
  else
    Writeln('[HorseMormotTest] Some tests FAILED — see details above.');

  ExitCode := GFailCount;

  Writeln('');
  Writeln('[HorseMormotTest] Press ENTER to exit...');
  Readln;
end.
