program HorseMormotTLSTestClient;

{$APPTYPE CONSOLE}

{
  Horse + mORMot2  —  TLS / mutual-TLS test client
  =================================================
  Destination: horse-provider-mormot/samples/tests/HorseMormotTLSTestClient.dpr

  Drives HorseMormotTLSTestServer over HTTPS (https://127.0.0.1:9201) with
  TCrossHttpClient (the same HTTP driver the mORMot param test client uses).
  Two modes, matching the server's argument:
...
begin
  Writeln('Horse + mORMot2  —  TLS / mutual-TLS test');
